...
class Options():
    def __init__(self):
        ...
        self.intermidiate_result_root = '../data/gan/cta2dwi/case_178_forHuang_rescale/intermidiate_result_{}'.format(__file__.split('.')[0])
    ...
    for epoch_i in range(opt.epochs):
        for index, (ct_imgs, dwi_imgs) in enumerate(dataloader):
            ...
            if index%opt.display == 0:
                print('====> epochs:[{}][{}/{}]\tgan loss:[{:.3f}]\tdiscriminator loss:[{:.3f}]\tpatches discriminator loss:[{:.3f}]'.format(
                    epoch_i, index, len(dataloader), gan_model.loss_G.detach().cpu().numpy(), 
                    gan_model.loss_D.detach().cpu().numpy(), gan_model.loss_D_patch.detach().cpu().numpy()
                ))
