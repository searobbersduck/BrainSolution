...
from gan.losses.gan_loss import PerceptionLoss
...
class Pix2PixModel():
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterion_perception_loss = PerceptionLoss(self.device)
            ...
            if self.opt.patch_D:
                ...
                self.netD_P_cpu = torch.nn.parallel.DistributedDataParallel(self.netD_P, device_ids=[local_rank], output_device=local_rank)
    ...
    def backward_G(self):
        ...
        if self.mask is None and self.lbp_mask is None:
            self.loss_G_L1 = self.criterionL1(self.fake_B, self.real_B) * self.opt.lambda_L1 + 0.1*self.criterion_perception_loss(self.real_B, self.fake_B)
