...
'''
ref: https://github.com/matterport/Mask_RCNN/blob/master/mrcnn/utils.py
'''
...
def compute_iou_3d(box, boxes, box_volume, boxes_volume):
    ...
    box_volume: float. the area of 'box'
...
def non_max_suppression_2d(boxes, scores, threshold):
    ...
    while len(ixs) > 0:
        ...
        iou = compute_iou_2d(boxes[i], boxes[ixs[1:]], area[i], area[ixs[1:]])
...
def non_max_suppression_3d(boxes, scores, threshold):
    ...
    while len(ixs) > 0:
        ...
        pick.append(i)
        # Compute IoU of the picked box with the rest
        iou = compute_iou_3d(boxes[i], boxes[ixs[1:]], area[i], area[ixs[1:]])
        # Identify boxes with IoU over the threshold. This
        # returns indices into ixs[1:], so add 1 to get
        # indices into ixs.
        remove_ixs = np.where(iou > threshold)[0] + 1
